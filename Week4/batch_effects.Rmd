...
suppressPackageStartupMessages({
  ...
  library(Biobase)
  library(sva)
  library(bladderbatch) # BiocManager
  library(broom)
  library(tidyverse)
})
```
...
pdf('figures/Dabrowska_problem1.pdf', height=4,width=8)

# Create the table
table <- pheno %>%
  count(outcome, cancer, batch) %>%
  pivot_wider(names_from = batch, values_from = n, values_fill = 0) %>%
  rename_with(~ paste0("Batch ", .), -c(outcome, cancer)) %>%
  select(outcome, cancer, order(as.numeric(gsub("Batch ", "", names(.)[-(1:2)]))) + 2)

# Description text
description <- "This table summarizes the count of outcome and tumor presence combinations, categorized by batch. The first
two columns represent the combination of variables being observed, while the remaining columns correspond to different
batch numbers. Each row indicates the count of a specific outcome and tumor presence combination.

As visible in the table, there is a significant batch effect, with batches 1, 3, and 4 containing data exclusively for
mTCC, Normal, and Biopsy, respectively. Batches 2 and 5 show a more balanced distribution but still have a dominant
phenotype that constitutes the largest fraction of their respective data. In batch 2, sTCC-CIS makes up 72% of the batch,
while in batch 5, sTCC+CIS accounts for 63% of the batch."


table_plot <- tableGrob(table)
description_text <- textGrob(description, gp = gpar(fontsize = 8))


grid.arrange(table_plot, description_text)
...
```{r}
ordered_pheno <- pheno[order(pheno$batch),]
ordered_edata <- edata[, rownames(ordered_pheno)]

cleared_edata <- ComBat(
  dat=ordered_edata,
  batch=ordered_pheno$batch,
  mod=model.matrix(~1, data=ordered_pheno),
...
png("figures/Dabrowska_problem2_1.png",height=700,width=700)
heatmap.2(edata,
          main = "Bladder data", # heat map title
          notecol="black",      # change font color of cell labels to black
          density.info="none",  # turns off density plot inside color legend
          trace="none",         # turns off trace lines inside the heat map
          margins =c(12,9),     # widens margins around plot
...
```{r}
png("figures/Dabrowska_problem2_2.png",height=700,width=700)
heatmap.2(cleared_edata,
          main = "Bladder cleaned using ComBat", # heat map title
          notecol="black",      # change font color of cell labels to black
          density.info="none",  # turns off density plot inside color legend
          trace="none",         # turns off trace lines inside the heat map
          margins =c(12,9),     # widens margins around plot
...
```{r}
cor_matrix <- cor(cleared_edata, method = "pearson")
labels_p3 <- paste0("Batch no. ", pheno$batch, ", ", pheno$outcome)

# Create a heatmap of Pearson correlations
png("figures/Dabrowska_problem3.png", height=700, width=700)
heatmap.2(cor_matrix,
          main = "Pearson Correlation of Samples", 
          notecol="black",      
          density.info="none",  
          trace="none",         
          margins = c(12,9),     
...
edata_b <- as.matrix(exprs(bottomly.eset))
edata_b <- edata_b[rowMeans(edata_b) > 10, ]
edata_b <- log2(as.matrix(edata_b) + 1)

pheno_b <- pData(bottomly.eset)
```

```{r}
model_uncorrected <- lm(t(edata_b) ~ as.factor(pheno_b$strain) + as.factor(pheno_b$experiment.number))

model1_tidy <- tidy(model_uncorrected)

png("figures/Dabrowska_problem4_uncorr_hist.png", height=700, width=700)
# histogram of estimates of particular regression terms.
ggplot(model1_tidy %>% filter(term == "as.factor(pheno_b$strain)DBA/2J")) + geom_histogram(aes(x=estimate), bins = 100, fill="darkorange")
dev.off()

png("figures/Dabrowska_problem4_uncorr_coeff.png", height=700, width=700)
# different way of looking at coefficients from many different models
# the vertical line indicates the zero coefficient.
ggplot(model1_tidy, aes(estimate, term)) +
     geom_point() +
     geom_vline(xintercept = 0)
dev.off()
```
...
```{r}
edata_b_cleaned <- ComBat(
  dat=edata_b,
  batch=pheno_b$experiment.number,
  mod=model.matrix(~1, data=pheno_b),
  par.prior=TRUE,
  prior.plots=TRUE
  )

model_corrected <- lm(t(edata_b_cleaned) ~ as.factor(pheno_b$strain))
model2_tidy <- tidy(model_corrected)

png("figures/Dabrowska_problem4_corr_hist.png", height=700, width=700)
# histogram of estimates of particular regression terms.
ggplot(model2_tidy %>% filter(term == "as.factor(pheno_b$strain)DBA/2J")) + geom_histogram(aes(x=estimate), bins = 100, fill="darkorange")
dev.off()

png("figures/Dabrowska_problem4_corr_coeff.png", height=700, width=700)
# different way of looking at coefficients from many different models
# the vertical line indicates the zero coefficient.
ggplot(model2_tidy, aes(estimate, term)) +
     geom_point() +
     geom_vline(xintercept = 0)
dev.off()
```
...
```{r}
mod = model.matrix(~as.factor(strain),data=pheno_b)
mod0 = model.matrix(~1, data=pheno_b)

sva_out = sva(edata_b, mod, mod0, n.sv=num.sv(edata_b, mod, method="leek"))

modsva = lm(t(edata_b) ~ as.factor(pheno_b$strain) + sva_out$sv)
modsva_tidy <- tidy(modsva)
```

```{r}
est_compare <- tibble(
  ComBat = model2_tidy %>% filter(term == "as.factor(pheno_b$strain)DBA/2J") %>% select("estimate") %>% unlist,
  SVA = modsva_tidy %>% filter(term == "as.factor(pheno_b$strain)DBA/2J") %>% select("estimate") %>% unlist)

png("figures/Dabrowska_problem5_coeff.png", height=700, width=700)
ggplot(est_compare, aes(x=ComBat, y=SVA)) +
     geom_point(col="darkgrey", alpha=.5, size=.5) + geom_abline(intercept=0, slope=1, col="darkred") + geom_smooth(method = "lm", se = TRUE)  + theme_bw()
dev.off()
```
...
```{r}
png("figures/Dabrowska_problem5_pvals.png", height=700, width=1000)
pvalues <- tibble(
  ComBat = model2_tidy %>% filter(term == "as.factor(pheno_b$strain)DBA/2J") %>% select("p.value") %>% unlist, # from HW4
  SVA = modsva_tidy %>% filter(term == "as.factor(pheno_b$strain)DBA/2J") %>% select("p.value") %>% unlist)
