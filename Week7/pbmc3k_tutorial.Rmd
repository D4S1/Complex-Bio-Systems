...
pbmc.data <- Read10X(data.dir = "/home/hoshi/ME/Coding/NEO/Week7/filtered_gene_bc_matrices/hg19/")
...
```{r}
# Load 
data <- Read10X('/home/hoshi/ME/Coding/NEO/Week7/Zheng/hg19')
so <- CreateSeuratObject(counts = data, project = "Zheng", min.cells=3, min.features=200)
```
...
VlnPlot(
      so, 
      features = c("percent_mito", "nFeature_RNA", "nCount_RNA"), 
      pt.size=0,
      ncol=3
    )
...
```{r}
# odchudzanie
selected_cells <- colnames(so)[1:15000]
so <- subset(so, cells = selected_cells)
```
...
so <- RunPCA(so, features = var.genes)
```
...
so <- FindClusters(so, resolution = 0.4)
```

```{r}
# Visualization
so <- RunTSNE(so, dims = 1:9)
DimPlot(so, label = TRUE, group.by = 'seurat_clusters', reduction = 'tsne') + NoLegend()
```
```{r clusterHeatmap, fig.height=8, fig.width=15}
...
Table created basez on Zheng 2017 and ChatGPT

Cluster ID | Markers       | Cell Type
-----------|---------------|----------
0          | LEF1, CCR7    | Naive CD4 T-cell
1          | AQP3, CORO1B  | Memory T cells
2          | GZMK, DUSP2   | Naive cytotoxic cell
3          | GZMH, ZNF683  | CD8+ cytotoxic T cell
4          | IGFBP7, AKR1C3| NK
5          | VPREB3, FCER2 | B cells
6          | S100A8, LGALS2| Classical monocytes
7          | C1QA, LILRA3  | Macrophages
8          | LRRC26, SCT   | Megakaryocytes
9          | TMEM40, ITGB3 | Dendritic

```{r}
celltype <- c("Naive CD4 T-cell", "Memory T cells", "Naive cytotoxic cell", "CD8+ cytotoxic T cell", "NK", "B cells", "Classical monocytes", "Macrophages", "Megakaryocytes", "Dendritic")
names(celltype) <- levels(so)
so@meta.data$celltype <- celltype[Idents(so)]
DimPlot(so, reduction = 'tsne', label = TRUE, group.by='celltype', pt.size = 0.5) + NoLegend()
```

```{r fig.height=8, fig.width=15}
FeaturePlot(so, ncol=3, features = c("CD3D", "CD8A", "NKG7", "FCER1A", "FCGR3A", "S100A8"))
```
...
```{r}
subpopulation_labels <- rep(NA, ncol(so))
names(subpopulation_labels) <- colnames(so)


for (cluster_id in 0:9) {
  
  cluster_cells <- WhichCells(so, expression = seurat_clusters == cluster_id)
  pca_data <- Embeddings(so, "pca")[cluster_cells, 1:10]
  
  # Solution adapted from ChatGPT sugestion
...
# Save to metadata
seurat_obj$subpopulation <- subpopulation_labels
```

```{r fig.height=8, fig.width=15}

tsne_coords <- as.data.frame(Embeddings(so, reduction = "tsne"))

tsne_coords$seurat_cluster <- so@meta.data$seurat_cluster
tsne_coords$subpopulation <- so@meta.data$subpopulation


centers <- tsne_coords %>%
  group_by(seurat_cluster) %>%
  summarise(x = median(tSNE_1),
            y = median(tSNE_2))


ggplot(tsne_coords, aes(x = tSNE_1, y = tSNE_2, color = subpopulation)) +

  geom_point(size = 1.5, alpha = 0.8) +
  geom_text(data = centers, aes(x = x, y = y, label = seurat_cluster),
            color = "black", size = 5, fontface = "bold") +
  theme_classic() +
  labs(title = "tSNE Plot with Cluster Labels",
       x = "tSNE 1",
       y = "tSNE 2")
```

```{r fig.height=8, fig.width=15}
DimPlot(so, reductio='tsne', split.by ='seurat_clusters', group.by = 'subpopulation', ncol=3)
```
